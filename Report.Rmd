...
We plot the distribution of the predictors separated by the benign and malignant classes, and observe again that predictors associated with texture, smoothness, symmetry and fractal dimension are inseparable and therefore might not be helpful for the classification problem. For example, the distributions of `smoothness_se`  for benign and malignant cancers almost completely overlap, same do `symmetry_se`, `fractal_dimension_se` and `texture_se`. Therefore, we will consider filter out these predictors in our final model.
...
dat[1:8]%>%kable(digits=4)
dat[9:15]%>%kable(digits=4)
...
![Benign (1)](pics/IJABMR-3-22-g001.jpeg){height=50%, width=50%}

![Benign (2)](pics/IJABMR-3-22-g002.jpeg){height=50%, width=50%}
...
![Malignant](pics/IJABMR-3-22-g004.jpeg){height=50%, width=50%}
