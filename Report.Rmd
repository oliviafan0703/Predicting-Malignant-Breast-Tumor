...
---
title: "Non-invasive Breast Tumor Diagnosis in Machine Learning"
...
The original dataset contains a blank column `...33`, so we dropped it. We also dropped the `id` column, and rename several columns, `concave points_mean`, `concave points_worst` and `concave points_se` that contains blank space in their names. 
...
```{r, echo=F, warning=F, fig.height = 1.5, fig.width = 1.5, fig.cap = "Distribution of Malignant and Benign Tumors"}
...
We divide the data into 3 categories according to their features, namely, mean, standard error and worst. Based on the correlation matrix (Figure 2), we have several major observations: 
...
We plot the distribution of the predictors separated by the benign and malignant classes (Figure 4), and observe again that predictors associated with texture, smoothness, symmetry and fractal dimension are inseparable and therefore might not be helpful for the classification problem. For example, the distributions of `smoothness_se`  for benign and malignant cancers almost completely overlap, same do `symmetry_se`, `fractal_dimension_se` and `texture_se`. Therefore, we will consider filter out these predictors in our final model.
...
We decided to use a LASSO-penalized logistic regression model to perform variable selection by gauging insights into which predictors are the most contributive, since less significant variables are forced to be exactly zero, and the most significant variables are kept in the final model and also since LASSO is a great tool dealing with multicollinearity issue. Since our EDA shows that many of the input variables are correlated, LASSO will drop the highly correlated features.
...
df%>%filter(s0!=0)%>%rename("coefficient"=s0)%>%kable(digits=4,caption = "LASSO-Penalized Logistic Model")
```
...
We tune the hyperparameter cost through cross validation considering a range of values from 0.001 to 100, and selected the optimal cost = 5 
...
```{r, echo=F, warning=F, fig.height = 3.6, fig.width = 4.6}
...
```{r, echo=F, warning=F, fig.height = 3.5, fig.width = 4.6}
library(pROC)
...
```{r, echo=F, warning=F, fig.height = 3, fig.width = 4, fig.cap = "Importance in predictors in RF model"}
...
![Benign Tumor(1)](pics/IJABMR-3-22-g001.jpeg){height=35%, width=35%}

![Benign Tumor(2)](pics/IJABMR-3-22-g002.jpeg){height=35%, width=35%}
...
![Malignant tumor](pics/IJABMR-3-22-g004.jpeg){height=35%, width=35%}
